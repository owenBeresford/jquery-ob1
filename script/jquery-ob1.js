...
(function($){
	...
	$.ob1Impl = function(el, options) {
		...
		function CorrectionModule(el, options) {
			...
			if($.fn.readingDuration && location.pathname != '/resource/home') {
				$('.h4_page').readingDuration({ dataLocation:"#main", target:".addReading", debug:this.options.debug, linkTo:'//owenberesford.me.uk/resource/jQuery-reading-duration'});
			}
	...
	if(typeof biblio_loader == 'function') {
		...
		var id='.lotsOfWords';
		if($(id+" sup a").length===0) { id='.halferWords'; }
		if($(id+" sup a").length>3) { 
			if(this.options.debug) { console.log("Have quite a few references, will extract for mobile."); }
			...
			$.ajax({ 
type: 'HEAD', async:false, url:url, timeout:1000, success:function( data, textStatus, jqXHR) {
			if(this.options.debug) { console.log("Have references cache, applying it."); }
$('document').biblio( {tocEdit:1, width:500, debug:this.options.debug, loosingElement:$id, extendViaDownload:4, referencesCache:url } ); 
} , error:function(jqXHR, textStatus, errorThrown ) {
if(this.options.debug) { console.log("Doing a manual download."); }
$('document').biblio( {tocEdit:1, debug:this.options.debug, loosingElement:$id, extendViaDownload:2 } ); 
}
}); 
...
// suppress columnising for a single column, or the <HR> come up wonky
		...
		if(jQuery.fn.wresize) {
			...
			$t.wait( function( this.columnise(); )); 
		}
...
// 	var $offset	= 30 + t1 + t2; // // with the current build, the larger figure  seems too large
	...
	if(!this.options.resize_registered) {
		$(window).resize(function() { this.alignHeader(); });
...
Date.prototype.importDate=function(format, day, time) {
  ...
  while(fpos<format.length) {
    ...
    case 'y': { year1 = parseInt(day1[bpos], 10); break; }
    case 'm': { month1 = parseInt(day1[bpos], 10); month1--; break; }
    case 'd': { _day1 = parseInt(day1[bpos], 10); break; }
    case 'h': { hour1 = parseInt(day1[bpos], 10); hour1--; break; }
    case 'i': { min1 = parseInt(day1[bpos], 10); break; }
    case 's': { sec1 = parseInt(day1[bpos], 10); break; }
    }
    fpos++;
    bpos++;
  }
